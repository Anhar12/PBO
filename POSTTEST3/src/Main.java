...
public class Main {
    ...
    static ArrayList<VariasiTopUp> dataGame = new ArrayList<>();
    ...
    // cek game yang sama
    static public boolean cek_game(String game){
        for (int i = 0 ; i < dataGame.size() ; i++){
            if (dataGame.get(i).getGame().equals(game)){
                return false;
            }
        }
        return true;
    ...
    static public void geser(int panjang){
    ...
    static void display_game(){
        ...
        System.out.println("===================================================");
        for (int i = 0 ; i < dataGame.size() ; i++){
            int jarak = dataGame.get(i).getGame().length()/2;
            System.out.print("  [" + (i+1) + "]");
            geser(20-jarak);
            System.out.println(dataGame.get(i).getGame());
            System.out.println("---------------------------------------------------");
            for (int j = 0 ; j < dataGame.get(i).getJenisGame().size() ; j++){
                System.out.printf("%15d %s", dataGame.get(i).getJenisGame().get(j).get(0), dataGame.get(i).getMataUang());
                System.out.printf("%10s%d %n", "\tRp", dataGame.get(i).getJenisGame().get(j).get(1), dataGame.get(i).getMataUang());
            }
            System.out.println("===================================================");
        }
    }

    static void display_pesanan(){
        cls();
        System.out.println("==================================================================================================================================");
        ...
        for (int i = 0 ; i < dataPesanan.size() ; i++){
            ...
            String jumlahTopUpS = String.valueOf(dataPesanan.get(i).getJumlahTopUP());
            String TotalS = String.valueOf(dataPesanan.get(i).getTotal());
            System.out.print("| " + iS);
            geser(4-iS.length());
            System.out.print("| " + dataPesanan.get(i).getGame());
            geser(23-dataPesanan.get(i).getGame().length());
            System.out.print("| " + dataPesanan.get(i).getID());
            geser(14-dataPesanan.get(i).getID().length());
            System.out.print("| " + dataPesanan.get(i).getNickname());
            geser(16-dataPesanan.get(i).getNickname().length());
            System.out.print("| " + dataPesanan.get(i).getJumlahTopUP() + " " + dataPesanan.get(i).getMataUang());
            geser(15-jumlahTopUpS.length()-dataPesanan.get(i).getMataUang().length());
            System.out.print("| Rp" + dataPesanan.get(i).getTotal());
            geser(14-TotalS.length());
            System.out.print("| " + dataPesanan.get(i).getWaktu());
            geser(26-dataPesanan.get(i).getWaktu().length());
            System.out.println("|");
            System.out.println("----------------------------------------------------------------------------------------------------------------------------------");       
        }
    }
            ...
            switch (pilih){
                            ...
                            while (loop){
                                ...
                                while (true){
                                    ...
                                    if (ulang.equalsIgnoreCase("n") || ulang.equalsIgnoreCase("N")){
                                        VariasiTopUp game_baru = new VariasiTopUp(game, mataUang, jenis);
                ...
                case "3":
                    if (dataGame.size() > 0){
                        try{
                            ...
                            System.out.println("==================================================");
                            display_game();
                            System.out.print("Ingin Edit Data Nomor Berapa : "); int edit = Integer.parseInt(br.readLine());
                            if (edit > 0 && edit <= dataGame.size()){
                                String game_edit;
                                ...
                                while (loop_edit) {
                                    ...
                                    switch(pilih){
                                        ...
                                        while (true){
                                            ...
                                            if (cek_game(game_edit) == true){
                                                dataGame.get(edit-1).setGame(game_edit);
                                                System.out.print("\nData Berhasil Diedit! Tekan ENTER Untuk Melanjutkan..."); gtc = br.readLine();
                                                break;
                                            } else {
                                    ...
                                    case "2":
                                        ...
                                        if (dataGame.get(edit-1).cek_variasi(cari_edit) == true){
                                            ...
                                            dataGame.get(edit-1).set_topUp(data_edit, dataGame.get(edit-1).idx_variasi(cari_edit));;
                                        } else {
                                    ...
                                    case "3":
                                        while (true){
                                            ...
                                            if (cek_game(game) == true){
                                                System.out.print("Masukkan Mata Uang Game Yang Baru : "); String mataUang = br.readLine();
                                                while (edit_variasi){
                                                    System.out.print("Masukkan Variasi Top Up Yang Baru : "); variasi_edit = Integer.parseInt(br.readLine());
                                                    System.out.print("Masukkan Harga Top Up Yang Baru   : Rp"); harga_edit = Integer.parseInt(br.readLine());
                                                    data_edit.add(variasi_edit);
                                                    ...
                                                    while (true){
                                                        System.out.print("\nTambah Variasi Top Up Lagi [Y/N] ? "); String ulang = br.readLine();
                                                        if (ulang.equalsIgnoreCase("n") || ulang.equalsIgnoreCase("N")){
                                                            dataGame.get(edit-1).setGame(game);
                                                            dataGame.get(edit-1).setMataUang(mataUang);
                                                            dataGame.get(edit-1).update(data_edit);
                                                            edit_variasi = false;
                                                            System.out.print("\nData Berhasil Diedit! Tekan ENTER Untuk Melanjutkan..."); gtc = br.readLine();
                                                            break;
                                                        } else if (ulang.equalsIgnoreCase("y") || ulang.equalsIgnoreCase("Y")){
                ...
                case "4":
                    if (dataGame.size() > 0){
                        try{
                            ...
                            System.out.println("===================================================");
                            display_game();
                            System.out.print("Ingin Hapus Data Nomor Berapa : "); int hapus_game = Integer.parseInt(br.readLine());
                            if (hapus_game > 0 && hapus_game <= dataGame.size()){
                                while(true){
                                    cls();
                                    System.out.println("===================================================");
                                    System.out.println("            >>    Data Ditemukan    <<             ");
                                    System.out.println("===================================================");
                                    dataGame.get(hapus_game-1).display();
                ...
                case "5":
                    if (dataGame.size() > 0){
                        ...
                        for (int i = 0 ; i < dataGame.size() ; i++){
                            if (dataGame.get(i).getGame().equals(cari)){
                                cls();
                                System.out.println("===================================================");
                                System.out.println("            >>    Data Ditemukan    <<             ");
                                System.out.println("===================================================");
            ...
            switch(pilih){
                ...
                case "2":
                    ...
                    while (true){
                        ...
                        System.out.println("===================================================");
                        try {
                            System.out.print("Mau Order Game Nomor Berapa : "); int idx_game = Integer.parseInt(br.readLine());
                            idx_game -= 1;
                            if (idx_game >= 0 && idx_game < dataGame.size()){
                                while (true){
                                    ...
                                    if (dataGame.get(idx_game).cek_variasi(TopUp) == true){
                                        ...
                                        String game_topup = dataGame.get(idx_game).getGame();
    ...
    public static void main(String[] args) throws IOException {
        ...
        VariasiTopUp gameDefault = new VariasiTopUp("Fifa Mobile", "POINTS", jenisDefault);
